...
object Frames {
    ...
    fun render(shot: Shot, bg: Bitmap?, scale: Float): Bitmap {
        val (w, h) = Shot.size(shot.style)
        ...
        when (shot.style) {
            Shot.SHORTS -> shorts(c, shot, w, h)
            Shot.PLAYER -> player(c, shot, w, h)
            Shot.THUMB -> thumb(c, shot, w, h)
    ...
    private fun drawBase(c: Canvas, s: Shot, bg: Bitmap?, w: Int, h: Int) {
    ...
    private fun grade(s: Shot): ColorMatrix {
        val cm = ColorMatrix()
    ...
    private fun shorts(c: Canvas, s: Shot, w: Int, h: Int) {
        val p = Paint(Paint.ANTI_ALIAS_FLAG)
    ...
    private fun player(c: Canvas, s: Shot, w: Int, h: Int) {
        val p = Paint(Paint.ANTI_ALIAS_FLAG)
    ...
    private fun timeText(s: Shot): String {
    ...
    private fun thumb(c: Canvas, s: Shot, w: Int, h: Int) {
        val p = Paint(Paint.ANTI_ALIAS_FLAG)
