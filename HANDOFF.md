...
| Cut: 区間の試聴 | 実装済み（MediaPlayer で in→out を再生） |
| Desk: 話速の実測較正 | 実装済み（Speed オブジェクト） |
| Desk: チャンネル設定 | 実装済み（Channel オブジェクト） |
...
### 話速（Speed）— v3.13
想定尺とチャプターの精度が**すべてこの係数に乗る**。既定 5.3 文字/秒は日本語の
一人喋りの一般値だが、人によって 4.0〜7.0 と幅がある。実測でしか正確な値は得られない。

- 台本タブの「話速を測る」で、本文を声に出して読んで測る
- `Speed.cps` は SharedPreferences に保存し、`MainActivity.onCreate` で `load()` する
- **`Project.seconds()` / `sceneSeconds()` は `Speed.cps` を参照する。**
  以前の `Project.CPS` 定数は `CPS_DEFAULT` に改名して既定値としてのみ残した
- 2.5〜12.0 の範囲に丸める。読み間違いで極端な値が入るのを防ぐため

### 区間の試聴（Cut）
無音カットの当たり外れは**音を聞かないと判断できない**。抽出フレームだけでは
語尾が切れているかが分からない。区間の詳細から in→out を MediaPlayer で再生する。
`onPause` と `onDestroy` で必ず停止・解放すること（バックグラウンドで鳴り続ける）。

### チャンネル設定（Desk）
`@名` を1箇所で持ち、画像タブの channel スロット初期値と説明文に流用する。
様式を切り替えるたびに打ち直していた手間をなくすため。
